...
@HytaleComponent
public class TestBlockInfoComponent implements ILifetimeComponent<ChunkStore> {
    ...
    public static final ComponentType<ChunkStore, TestBlockInfoComponent> getComponentType() {
        if (COMPONENT_TYPE == null) {
            COMPONENT_TYPE = LibHytale.getChunkComponentType(TestBlockInfoComponent.class);
    ...
    public static final void runTests(
        ...
        if (verbose != null) {
            Chat.log("");
            Chat.log("Added TEST_BlockInfo block");
        }
        ...
        var res = BlockUtils.Info.test(ref, worldChunk, commandBuffer, coords);
        ...
        if (verbose == null) {
            if (success == res.size() && allTheSame) {
                Chat.log("+ (2) SUCCESS: TEST_BlockInfo");
            } else {
                Chat.log("- (2) FAILED:  TEST_BlockInfo");
            }
        }
    }
